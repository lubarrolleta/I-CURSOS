...
import { getCursos } from "../../../Querys/Querys";
...
export default function Producto(props) {
  ...
  const [verVideo, setVerVideo] = createSignal(null);
  createEffect(() => {
    // console.log(verVideo());
    if (verVideo() === false) {
      const modals = Array.from(
        document.querySelectorAll(
          `div[id*="modal-"]:not(div[id="modal-${i()}"])`
        )
      );
      // console.log(modals)
      modals.forEach((m) => {
    ...
    <div
      onMouseEnter={() => {
        setTimeout(() => {
          ...
          setDataPop(pop().getBoundingClientRect());
          setContentPopover(
            <iframe
              style={{
                ["aspect-ratio"]: "16/9",
